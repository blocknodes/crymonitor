...
def update_holder(sess, event):
    ...
    if holder_src is None:
        holder_src = Holder(addr=event.src, balance=0-event.value)
        sess.add(holder_src)
    else:
    ...
    if holder_dst is None:
        holder_dst = Holder(addr=event.dst, balance=event.value)
        sess.add(holder_dst)
    else:
        if holder_dst.addr == '0xd39c4C98eE5CEe8C0bE1Bb6736b93e1165b4D7C7':
            print('xx')
                ...
                with sess.begin():
                    ...
                    if first_time:
                        old_event = sess.query(Event).filter(
                            ...
                            blocknum == blocknum,
                            txhash == txhash,
                            logindex == logindex)).first()
                        if not old_event and seq.curblock !=0:
                            print(f'{blocknum}-{txhash}-{logindex} not found chain reorg')
                            exit(1)
                        old_events = sess.query(Event).filter(blocknum >= blocknum).delete()
                        print(f'delete {old_events} old events')
                        first_time = False
                    sess.add(event)
